...
class BoardState {
    ...
    // Extracted method to register DOM event handlers
    registerDomEventHandlers() {
        ...
        const handlePressStart = (event) => {
            ...
            pressTimer = setTimeout(() => {
                isPressEvent = true;
                const pressEvent = new LongPressEvent('press', target, { row, col });
        ...
        const handlePressEnd = (event) => {
            ...
            if (!isPressEvent) {
                ...
                const row = target.parentElement.rowIndex;
                const col = target.cellIndex;
                const clickEvent = new ClickEvent('click', target, { row, col });
        ...
        this.table.addEventListener('touchstart', handlePressStart);
        this.table.addEventListener('mouseup', handlePressEnd);
        this.table.addEventListener('touchend', handlePressEnd);
    ...
    constructor(tableSelector, shipStatusBoard) {
        if (!this.table) {
            ...
            if (this.table) {
                this.registerDomEventHandlers(); // Register event handlers
            }
        }
